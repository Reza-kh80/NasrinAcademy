...
import { initialPublicTermStatusObject } from './teacher-form-objects';
function TeacherStatusAssignPublicForm(props) {
    const apiEndPoint = process.env.REACT_APP_APIEndPoint;
    const currentUser = useContext(UserContext);
    const simpleValidator = useRef(new SimpleReactValidator())
    const { show, onHide, studentId, statusId, title } = props;
    const [termStatusObject, setTermStatusObject] = useState(initialPublicTermStatusObject);
    const onEditHandler = useCallback((list) => {
        setTermStatusObject(prevState => ({
            ...prevState,
            PublicTermStatusId: list.PublicTermStatusId,
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && statusId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermStatus/GetPublic?studentId=${studentId}&termStatusId=${statusId}`)
                if (mounted) {
            ...
            setTermStatusObject(initialPublicTermStatusObject);
            setTermStatusObject(prevState => ({
                ...prevState,
                Modifier: currentUser.Username,
                StudentId: studentId,
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermStatus/AddPublic', termStatusObject, {})
                .then(response => {
...
export default TeacherStatusAssignPublicForm;
