...
function TeacherFlashCardForm(props) {
    ...
    if (id !== "") {
      const fetchData = async () => {
        const result = await axios.get(
          apiEndPoint + `FlashCard/Get?flashCardId=${id}`
        );
        if (mounted) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const result = await axios.get(
        apiEndPoint + "FlashCardCategory/DisplayDropdown"
      );
      if (mounted) {
        result.data.map((m) =>
          dropdownList.push({
  ...
  const handleSubmit = () => {
    if (simpleValidator.current.allValid()) {
      axios
        .post(apiEndPoint + "FlashCard/Add", flashCardObject, {})
        .then((response) => {
        ...
        <Modal.Body>
          <Container fluid>
            <Row>
              <Col xs="12" lg="4" className="text-left m-0 p-1">
                <Form>
                  ...
                  {simpleValidator.current.message(
                    "flashCardCategory",
                    flashCardObject.FlashCardCategoryId,
                    "required",
                    { className: "alert alert-danger" }
                  )}
                </Form>
              ...
              <Col xs="12" lg="4" className="align-items-center m-0 p-1">
                <Form.Group>
                  ...
                  >
                    {flashCardObject.ModificationDate}
                  </Form.Label>
                  <Form.Label htmlFor="modificationDate">
                ...
                <Form.Group>
                  <Form.Check
                    ...
                    name="IsDeleted"
                    label="Is Deleted?"
                    checked={flashCardObject.IsDeleted}
                ...
                {currentUser.RoleName === "Dean" && (
                  <Form.Group>
                    <Form.Check
                      ...
                      checked={flashCardObject.IsFree}
                      onChange={handleChange}
                    />
                  </Form.Group>
                )}
              ...
              <Col xs="12" lg="4" className="text-left m-0 p-1">
                <Form.Group>
                  <Form.Label htmlFor="color">
                    <h6 className="text-primary mb-0 ml-1">
                      Course backgroung Color:
                    </h6>
                  </Form.Label>
                  <ChromePicker
                    className="mb-2"
                    name="b"
                    color={flashCardObject.Color}
                    onChangeComplete={handleColorChange}
                    onBlur={simpleValidator.current.showMessageFor("color")}
                  />
                  {simpleValidator.current.message(
                    "color",
                    flashCardObject.Color,
                    "required",
                    { className: "alert alert-danger" }
                  )}
                </Form.Group>
            ...
            <Row>
              <Col xs="12" lg="6">
                <FileUpload
                  postMethod={"FlashCard/UploadFile"}
                  title="Audio(mp3,mpeg)"
                  accept={["mp3", "mpeg"]}
                  specifiedFileName="NoName"
                  onUpload={handleUploadAudio}
                  onBlur={simpleValidator.current.showMessageFor("audio")}
                />
                {flashCardObject.Audio !== "" && (
                  <audio
                    controls
                    preload="none"
                    src={mediaEndPoint + "FlashCard/" + flashCardObject.Audio}
                    width="100%"
                    height="100%"
                    type="audio/mpeg"
                    className="p-0"
              ...
              <Col xs="12" lg="6">
                <FileUpload
                  postMethod={"FlashCard/UploadFile"}
                  title="Photo(jpg,jpeg)"
                  accept={["jpeg", "jpg"]}
                  specifiedFileName="NoName"
                  onUpload={handleUploadPicture}
                  onBlur={simpleValidator.current.showMessageFor("picture")}
                />
                {flashCardObject.Picture !== "" && (
                  <img
                    className="img-card text-center"
                    width="200"
                    height="200"
                    src={mediaEndPoint + "FlashCard/" + flashCardObject.Picture}
                    onError={addDefaultSrc}
                    alt={flashCardObject.Picture + "picture"}
                  />
                )}
              </Col>
            </Row>
            ...
            <Row>
              <Col xs="12" lg="6" className="m-0 p-1">
                ...
                <Form.Group>
                  <Form.Label htmlFor="description">
                    <h6 className="text-primary mb-0 ml-1">Description:</h6>
                  </Form.Label>
                  <Form.Control
                    id="description"
                    name="Description"
                    ...
                    value={flashCardObject.Description}
                    onChange={handleChange}
                    onBlur={simpleValidator.current.showMessageFor(
                      "description"
                    )}
                ...
                <Form.Group>
                  <Form.Label htmlFor="example">
                    <h6 className="text-primary mb-0 ml-1">Examples:</h6>
                  </Form.Label>
                  <Form.Control
                    id="example"
                    name="Example"
                    ...
                    rows={10}
                    placeholder="Enter description"
                    value={flashCardObject.Example}
                    onChange={handleChange}
                    onBlur={simpleValidator.current.showMessageFor("example")}
                  />
                  {simpleValidator.current.message(
                    "example",
                    flashCardObject.Example,
                    "required|max:999",
                    { className: "alert alert-danger" }
                  )}
                </Form.Group>
              ...
              <Col xs="12" lg="6" className="m-0 p-1">
                <Form.Group dir="rtl" className="text-right">
                  <Form.Label htmlFor="titleTranslation text-right">
                    <h6 className="text-primary mb-0 ml-1 ">عنوان:</h6>
                  </Form.Label>
                  <Form.Control
                    id="titleTranslation"
                    ...
                    type="text"
                    placeholder="ترجمه"
                    value={flashCardObject.TitleTranslation}
                  ...
                  {simpleValidator.current.message(
                    "title translation",
                    flashCardObject.TitleTranslation,
                    "required|max:99",
                    { className: "alert alert-danger" }
                  )}
                </Form.Group>
                <Form.Group dir="rtl" className="text-right">
                  <Form.Label htmlFor="descriptionTranslation">
                    <h6 className="text-primary mb-0 ml-1">توضیحات:</h6>
                  </Form.Label>
                  <Form.Control
                    id="descriptionTranslation"
                    name="DescriptionTranslation"
                    as="textarea"
                    rows={10}
                    placeholder="ترجمه"
                    value={flashCardObject.DescriptionTranslation}
                    className="text-right"
                    onChange={handleChange}
                    onBlur={simpleValidator.current.showMessageFor(
                  ...
                  {simpleValidator.current.message(
                    "description translation",
                    flashCardObject.DescriptionTranslation,
                    "required|max:999",
                    { className: "alert alert-danger" }
                  )}
                </Form.Group>
                <Form.Group dir="rtl" className="text-right">
                  <Form.Label htmlFor="exampleTranslation">
                    <h6 className="text-primary mb-0 ml-1">مثالها:</h6>
                  </Form.Label>
                  <Form.Control
                    ...
                    rows={10}
                    placeholder="ترجمه"
                    value={flashCardObject.ExampleTranslation}
                    className="text-right"
                    onChange={handleChange}
                    onBlur={simpleValidator.current.showMessageFor(
                      "example translation"
                  ...
                  {simpleValidator.current.message(
                    "example translation",
                    flashCardObject.ExampleTranslation,
                    "required|max:999",
                    { className: "alert alert-danger" }
                  )}
                </Form.Group>
...
export default TeacherFlashCardForm;
