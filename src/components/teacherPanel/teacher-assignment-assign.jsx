...
class TeacherAssignmentAssign extends Component {
    ...
    getTermAssignment = async (id) => {
        const dataList = [];
        await axios.get(this.state.apiEndPoint + `TermAssignment/GetAllPrivate?studentId=${id}`).then(response => {
    ...
    onNewTermAssignment = async () => {
        ...
        const TeacherAssignmentAssignForm = await import('./forms/teacher-assignment-assign-form');
    ...
    render() {
        const { dataList, dataTitles, columnList, pageSize, filteredItem,
            title, showTermAssignmentRegister, studentList, teacherCourseList, selectedAssignmentId, selectedTeacherCourseId, selectedStudentId, TeacherAssignmentAssignForm } = this.state;
        return (<div>
            <Container fluid className="table">
                <Row className="mb-2">
                    <Col xs="12" className="text-left text-dark p-0" >
                        <h3 className="text-primary text-center font-weight-bold">Assign Private Assignments</h3>
                    </Col>
                </Row>
                <br />
                <Row>
...
export default TeacherAssignmentAssign;
