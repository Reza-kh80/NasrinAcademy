...
import { initialTermResultObject } from './teacher-form-objects';
function TeacherResultAssignForm(props) {
    ...
    const onEditHandler = useCallback((list) => {
        setTermResultObject(prevState => ({
            ...prevState,
            PrivateTermResultId: list.PrivateTermResultId,
            Code: list.Code,
            Title: list.Title,
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && resultId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermResult/GetPrivate?studentId=${studentId}&termResultId=${resultId}`)
                if (mounted) {
            ...
            setTermResultObject(initialTermResultObject);
            setTermResultObject(prevState => ({
                ...prevState,
                Modifier: currentUser.Username,
                StudentId: studentId,
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermResult/AddPrivate', termResultObject, {})
                .then(response => {
