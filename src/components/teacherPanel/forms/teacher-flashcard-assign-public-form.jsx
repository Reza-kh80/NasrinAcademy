...
function TeacherFlashCardAssignPublicForm(props) {
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && levelId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermFlashCard/GetPublic?studentId=${studentId}&levelId=${levelId}`)
                if (mounted) {
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermFlashCard/AddPublic', termFlashCardObject, {})
                .then(response => {
...
export default TeacherFlashCardAssignPublicForm;
