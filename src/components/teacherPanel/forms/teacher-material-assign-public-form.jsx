...
function TeacherMaterialAssignPublicForm(props) {
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && materialId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermMaterial/GetPublic?studentId=${studentId}&materialId=${materialId}`)
                if (mounted) {
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermMaterial/AddPublic', termMaterialObject, {})
                .then(response => {
...
export default TeacherMaterialAssignPublicForm;
