...
class DeanPrivateStudent extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            isRegistered: false,
            DeanPrivateStudentForm: null

        }
    }
    getNotRegisteredStudent = async () => {
        const dataList = [];
        await axios.get(this.state.apiEndPoint + 'Student/DisplayAllPrivateNotRegistered').then(response => {
    ...
    getRegisteredStudent = async () => {
        ...
        await axios.get(this.state.apiEndPoint + 'Student/DisplayAllPrivateRegistered').then(response => {
    ...
    onNewStudent = async () => {
        ...
        this.setState({ privateTeacherCourseId: 0 });
        this.setState({ isRegistered: false });
        this.setState({ title: 'Register New Student' });
        this.setState({ showStudentRegister: true });
        const DeanPrivateStudentForm = await import('./forms/dean-private-student-form');
        this.setState({ DeanPrivateStudentForm: DeanPrivateStudentForm.default });
    ...
    handleHideStudentRegister = () => {
        ...
        this.setState({ isRegistered: false })
        this.getRegisteredStudent();
        this.setState({ DeanPrivateStudentForm: null });
    }
    render() {
        const { dataList, dataTitles, columnList, pageSize, filteredItem, dataListStudent, dataTitlesStudent, columnListStudent,
            pageSizeStudent, filteredItemStudent, title, showStudentRegister, studentId, isRegistered, privateTeacherCourseId,
            DeanPrivateStudentForm
        } = this.state;
        return (<div>
            ...
            {DeanPrivateStudentForm &&
                <DeanPrivateStudentForm studentId={studentId} teacherCourseId={privateTeacherCourseId} isRegistered={isRegistered} title={title} show={showStudentRegister} onHide={this.handleHideStudentRegister} />
            }
        </div>);
    }
}
export default DeanPrivateStudent;
