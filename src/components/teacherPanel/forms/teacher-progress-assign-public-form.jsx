...
import { initialPublicTermProgressObject } from './teacher-form-objects';
function TeacherProgressAssignPublicForm(props) {
    const apiEndPoint = process.env.REACT_APP_APIEndPoint;
    const currentUser = useContext(UserContext);
    const simpleValidator = useRef(new SimpleReactValidator())
    const { show, onHide, studentId, progressId, title } = props;
    const [termProgressObject, setTermProgressObject] = useState(initialPublicTermProgressObject);
    const onEditHandler = useCallback((list) => {
        setTermProgressObject(prevState => ({
            ...prevState,
            PublicTermProgressId: list.PublicTermProgressId,
            Label: list.Label,
            Plan: list.Plan,
            Actual: list.Actual,
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && progressId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermProgress/GetPublic?studentId=${studentId}&termProgressId=${progressId}`)
                if (mounted) {
            ...
            setTermProgressObject(initialPublicTermProgressObject);
            setTermProgressObject(prevState => ({
                ...prevState,
                Modifier: currentUser.Username,
                StudentId: studentId,
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermProgress/AddPublic', termProgressObject, {})
                .then(response => {
...
export default TeacherProgressAssignPublicForm;
