...
import { initialPublicTeacherCourseObject } from './dean-form-objects';
const MyDropdown = React.lazy(() => import('../../general/dropdown'));
function DeanPublicTeacherCourseForm(props) {
    const apiEndPoint = process.env.REACT_APP_APIEndPoint;
    const currentUser = useContext(UserContext);
    const simpleValidator = useRef(new SimpleReactValidator())
    const { show, onHide, id, title } = props;
    const [teacherCourseObject, setTeacherCourseObject] = useState(initialPublicTeacherCourseObject);
    ...
    useEffect(() => {
        ...
        if (id !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TeacherCourse/GetPublic?teacherCourseId=${id}`)
                if (mounted) {
            ...
            setTeacherCourseObject(initialPublicTeacherCourseObject);
            setTeacherCourseObject(prevState => ({
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TeacherCourse/AddPublic', teacherCourseObject, {})
                .then(response => {
...
export default DeanPublicTeacherCourseForm;
