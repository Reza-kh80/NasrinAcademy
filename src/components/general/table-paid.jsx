...
function TableContent(props) {
    const { dataList, dataTitles, columnList, pageSize, filter, onEdithandler } = props;
    ...
    const itemList = () => {
        ...
        // console.log("Item=", itemList[0]["id"])
        return itemList;
    }
    const getColumn = (i) => {
            ...
            <Container fluid>
                <Row className="mb-2">
                    <Col xs="12" sm="6" className="p-0">
                        <Form.Control name={filter} type="text" placeholder="Search the Title" value={title} onChange={(e) => setTitle(e.currentTarget.value.toLowerCase())} />
                    </Col>
                </Row>
                <Row className="mb-2">
                                ...
                                {itemList().map((item, rowIndex) =>
                                    <tr key={rowIndex} className={(rowIndex % 2 === 0) ? "table-info" : "table-secondary"}>
                                        {getColumn(rowIndex).map((val, colIndex) =>
                                            typeof (val) !== "boolean"
                                                ?
                                                <td key={colIndex} className="text-center">{val.toString()} </td>
                                                :
                                                <td key={colIndex} className="text-center"><Form.Check inline defaultChecked={val} disabled /></td>
                                        )}
                                        {dataTitles.length > columnList.length &&
                                            <td className="text-center">
                                                <div>{ }</div>
                                                {getColumn(rowIndex)[6].toString() === "true" ?
                                                    <Button onClick={() => onEdithandler(item)} className="btn btn-sm" disabled>{dataTitles[dataTitles.length - 1]}</Button>
                                                    :
