...
function DeanPublicStudentForm(props) {
    ...
    const { show, onHide, title, studentId, isRegistered, termId } = props;
    const [studentObject, setStudentObject] = useState(initialPublicStudentObject);
    const [dropdownList, setDropdownList] = useState([]);
    const [publicTermId, setPublicTermId] = useState(termId);
    ...
    useEffect(() => {
        ...
        setPublicTermId(termId);
    }, [studentObject, termId]);
    useEffect(() => {
        let list = [];
        let mounted = true;
        const fetchData = async () => {
            const result = await axios.get(apiEndPoint + `Term/PublicTermDropdown`)
            if (mounted) {
    ...
    useEffect(() => {
        let mounted = true;
        if (studentId === 0) {
            setStudentObject(initialPublicStudentObject);
            setStudentObject(prevState => ({
            ...
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `Student/GetPublic?studentId=${studentId}`)
                if (mounted) {
    ...
    const handleDropdownSelect = (e) => {
        let publicTermId = e.target.value;
        if (publicTermId === "" || publicTermId === null || publicTermId === undefined) {
            setPublicTermId(0);
            return;
        }
        else {
            setPublicTermId(publicTermId);
            setStudentObject(prevState => ({
                ...prevState,
                PublicTermId: publicTermId
            }));
        }
    }
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'Student/AddPublic', studentObject, {})
                .then(response => {
                                ...
                                <Form>
                                    ...
                                    <Form.Group>
                                        <MyDropdown
                                            ...
                                            name="language"
                                            selectedValue={publicTermId}
                                            htmlFor="publicTermId"
                                            width={null}
                                        />
                                        <Form.Check inline name="IsDeleted" label="Is Deleted?" checked={studentObject.IsDeleted} onChange={handleChangeStudent} />
...
export default DeanPublicStudentForm;
