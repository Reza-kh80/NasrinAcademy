...
const CourseRequestPrivate = React.lazy(() => import('./course-request-private'));
const CourseOverview = React.lazy(() => import('./course-overview'));
function RegisterPrivate(props) {
    const lang = localStorage.getItem('lang')
    const { show, onHide, title, courseId, privateTeacherCourseId } = props;
                    ...
                    <div dir={lang === 'fa' ? 'rtl' : 'ltr'} className={lang === 'fa' ? 'text-right m-0 p-0' : 'text-left m-0 p-0'}>
                        ...
                        >
                            ...
                            <Tab eventKey="3" tabClassName="font-weight-bold" className="tab-border p-3" title={getHeading(lang)[2]}>
                                <Suspense fallback={<Spinner color="success" />}>
                                    <CourseRequestPrivate teacherCourseId={privateTeacherCourseId} onHide={handleHideRegister} />
                                </Suspense>
                            </Tab>
                        </Tabs>
                    </div>
