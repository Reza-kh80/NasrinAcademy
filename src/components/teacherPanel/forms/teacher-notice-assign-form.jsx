...
import { initialTermNoticeObject } from './teacher-form-objects';
function TeacherNoticeAssignForm(props) {
    ...
    const onEditHandler = useCallback((list) => {
        setTermNoticeObject(prevState => ({
            ...prevState,
            PrivateTermNoticeId: list.PrivateTermNoticeId,
            Title: list.Title,
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && noticeId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermNotice/GetPrivate?studentId=${studentId}&termNoticeId=${noticeId}`)
                if (mounted) {
            ...
            setTermNoticeObject(initialTermNoticeObject);
            setTermNoticeObject(prevState => ({
                ...prevState,
                Modifier: currentUser.Username,
                StudentId: studentId,
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermNotice/AddPrivate', termNoticeObject, {})
                .then(response => {
...
export default TeacherNoticeAssignForm;
