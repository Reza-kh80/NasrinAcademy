...
import { initialTermPaymentObject } from './teacher-form-objects';
function TeacherPaymentAssignForm(props) {
    const apiEndPoint = process.env.REACT_APP_APIEndPoint;
    const currentUser = useContext(UserContext);
    const simpleValidator = useRef(new SimpleReactValidator())
    const { show, onHide, studentId, paymentId, title } = props;
    const [termPaymentObject, setTermPaymentObject] = useState(initialTermPaymentObject);
    const onEditHandler = useCallback((list) => {
        setTermPaymentObject(prevState => ({
            ...prevState,
            PrivateTermPaymentId: list.PrivateTermPaymentId,
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && paymentId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermPayment/GetPrivate?termPaymentId=${paymentId}`)
                if (mounted) {
            ...
            setTermPaymentObject(initialTermPaymentObject);
            setTermPaymentObject(prevState => ({
                ...prevState,
                Modifier: currentUser.Username,
                StudentId: studentId,
    ...
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermPayment/AddPrivate', termPaymentObject, {})
                .then(response => {
...
export default TeacherPaymentAssignForm;
