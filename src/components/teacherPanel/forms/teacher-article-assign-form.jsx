...
import { initialTermArticleObject } from './teacher-form-objects';
const MyDropdown = React.lazy(() => import('../../general/dropdown'));
function TeacherArticleAssignForm(props) {
    const apiEndPoint = process.env.REACT_APP_APIEndPoint;
    const currentUser = useContext(UserContext);
    const simpleValidator = useRef(new SimpleReactValidator())
    const { show, onHide, studentId, levelId, title } = props;
    const [termArticleObject, setTermArticleObject] = useState(initialTermArticleObject);
    const [levelList, setLevelList] = useState([]);
    const onEditHandler = useCallback((list) => {
        setTermArticleObject(prevState => ({
            ...prevState,
    ...
    useEffect(() => {
        // console.log("TermArticle Object=", termArticleObject)
    ...
    useEffect(() => {
        ...
        if (studentId !== "" && levelId !== "") {
            const fetchData = async () => {
                const result = await axios.get(apiEndPoint + `TermArticle/GetPrivate?studentId=${studentId}&levelId=${levelId}`)
                if (mounted) {
            ...
            setTermArticleObject(initialTermArticleObject);
            setTermArticleObject(prevState => ({
                ...prevState,
                Modifier: currentUser.Username,
                StudentId: studentId,
    ...
    const handleChange = (e) => {
        ...
        if (e.target.type === 'checkbox') {
            setTermArticleObject(prevState => ({ ...prevState, [name]: checked }));
        }
        else {
            setTermArticleObject(prevState => ({ ...prevState, [name]: value }));
        }
    }
    const handleSubmit = () => {
        if (simpleValidator.current.allValid()) {
            axios.post(apiEndPoint + 'TermArticle/AddPrivate', termArticleObject, {})
                .then(response => {
                        ...
                        <Row>
                            <Col xs="12" className="align-items-center m-0 p-1">
                                <MyDropdown
                                    ...
                                    name="LevelId"
                                    selectedValue={termArticleObject.LevelId}
                                    htmlFor="time"
                                    onBlur={simpleValidator.current.showMessageFor('level')}
                                    width="w-100"
                                />
                                {simpleValidator.current.message('level', termArticleObject.LevelId, 'required', { className: 'alert alert-danger' })}
                                <Form.Group>
                                    <Form.Label htmlFor="modificationDate"><h6 className="text-primary mb-0 ml-1" >Modefication Date (Now):</h6></Form.Label>
                                    <Form.Label id="modificationDate" className="border rounded border-dark d-block p-2">{termArticleObject.ModificationDate}</Form.Label>
                                    <Form.Label htmlFor="modificationDate"><h6 className="text-primary mb-0 ml-1" >Modefier (Now):</h6></Form.Label>
                                    <Form.Label id="modificationDate" className="border rounded border-dark d-block p-2">{termArticleObject.Modifier}</Form.Label>
                                </Form.Group>
                                <Form.Group>
                                    <Form.Check inline name="IsDeleted" label="Is Deleted?" checked={termArticleObject.IsDeleted} onChange={handleChange} className="mt-2" />
                                </Form.Group>
                            </Col>
                        </Row>
...
export default TeacherArticleAssignForm;
